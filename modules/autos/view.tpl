		...
		<div class="col-md-6">
			<div class="v-item__header">
				...
				<div class="v-item-header__info">
					...
					<span class="v-item__header__info__item">{lang key='field_auto_condition'}: <a href="{$searchPageUrl}condition:{$item.condition}/"><b>{lang key="field_auto_condition+{$item.condition}"}</b></a></span>
				...
				<div class="ia-car-info ia-car-info--block">
					...
					{if $item.engine}
						<span class="ia-car-info__icon ia-car-info__icon--engine">{lang key='field_auto_engine'}: <b>{lang key="field_auto_engine+{$item.engine}"}{if $item.engine_type} {lang key="field_auto_engine_type+{$item.engine_type}"}{/if}{if $item.engine_size} {$item.engine_size}{/if}</b></span>
					{/if}
			...
			<div class="v-item-info__section">
				...
				<div class="v-item-features">
					{foreach explode(',', $item.options_features) as $opt}
						<div class="v-item-features__item"><span class="fa fa-check-square"></span> {lang key="field_auto_options_features+{$opt}"}</div>
					{/foreach}
				</div>
			</div>
		{/if}
